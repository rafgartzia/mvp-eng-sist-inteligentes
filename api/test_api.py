...
def test_add_patient_prediction(client, sample_patient_data):
    ...
    session = Session()
    existing_patient = session.query(Paciente).filter(
        Paciente.name == sample_patient_data['name']).first()
    ...
    assert data['systolic_bp'] == sample_patient_data['systolic_bp']
    assert data['diastolic_bp'] == sample_patient_data['diastolic_bp']
    assert data['blood_sugar'] == sample_patient_data['blood_sugar']
    assert data['body_temp'] == sample_patient_data['body_temp']
    assert data['heart_rate'] == sample_patient_data['heart_rate']

    # Verifica se a predição foi feita (outcome deve estar presente)
    assert 'risk_level' in data
    # Deve ser 0 (não diabético) ou 1 (diabético)
...
def test_get_nonexistent_patient(client):
    ...
    data = json.loads(response.data)
    # Note: há um typo no código original ("mesage" em vez de "message")
...
def cleanup_test_patients():
    ...
    test_patients = session.query(Paciente).filter(
        Paciente.name.in_(['Maria Oliveira'])
    ).all()
